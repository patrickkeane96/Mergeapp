...
export function BusinessDaysCalculator() {
  ...
  const generateAiSummary = async () => {
    try {
      ...
      // Instead of generating PDF, send timeline data directly
      const timelineData = results.map(day => ({
        event: day.event,
        day: day.day,
        date: day.date.toDateString(),
        dayOfWeek: day.dayOfWeek,
        isStopClock: day.isStopClock,
        stopClockDuration: day.stopClockDuration,
        extensionDays: day.extensionDays,
        isPast: day.date < new Date()
      }));
      
      const inputParams = {
        phaseOption,
        filingDate: filingDate?.toDateString(),
        preAssessmentDays,
        stopClockEnabled,
        stopClockDate: stopClockDate?.toDateString(),
        stopClockDuration,
        commitmentsEnabled,
        commitmentPhase,
        extensionDuration,
        totalDays: results.length > 0 ? results[results.length - 1].day : 0
      };
      ...
      setAiError(`Failed to generate summary: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
                  ...
                  {aiSummary && !isGeneratingAi && (
                    <div className="flex space-x-2">
                      <Button 
                        ...
                        disabled={isGeneratingAi || isGeneratingFollowUp}
        ...
        <div className="lg:col-span-3">
          <Card className="h-full">
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle>Merger reform calculator</CardTitle>
              {results.length > 0 && (
                <Button 
                  variant="outline" 
                  size="sm" 
