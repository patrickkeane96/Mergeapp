// Script to update existing mergers to set the has_phase_2 field
...
async function updateMergersWithPhase2() {
  try {
    ...
    try {
      // We'll use a direct REST API call to add the column
      ...
      if (!response.ok) {
        console.log('Could not add column via REST API. This is expected - please add the column manually in the Supabase SQL Editor.');
      } else {
        console.log('Column has_phase_2 added successfully.');
      }
    } catch (err) {
      console.log('Error adding column:', err.message);
      console.log('Please add the column manually in the Supabase SQL Editor with:');
      console.log('ALTER TABLE mergers ADD COLUMN IF NOT EXISTS has_phase_2 BOOLEAN DEFAULT false;');
    }
